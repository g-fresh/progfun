...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def itr(set: TweetSet, list: TweetList): TweetList = {
      set match {
        case s: Empty => list
        case s: NonEmpty => {
          val tweet = s.mostRetweeted
          itr(s.remove(tweet), new Cons(tweet, list))
        }
      }
    }
    itr(this, Nil)
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    ...
    right.filterAcc(p, left.filterAcc(p, accumulator))
  }
  ...
  def mostRetweeted: Tweet = {
    val leftMin: Tweet = left match {
      case subset: NonEmpty => subset.mostRetweeted
      case _ => this.elem
    }
    val rightMin = right match {
      case subset: NonEmpty => subset.mostRetweeted
      case _ => this.elem
    }
    val m1 = if (leftMin.retweets < rightMin.retweets) leftMin else rightMin
    if (m1.retweets < elem.retweets) m1 else elem
  }
